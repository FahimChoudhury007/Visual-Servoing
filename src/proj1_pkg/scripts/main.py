...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    if task == 'line':
        trajectory = LinearTrajectory(current_position, target_position, 5)
    elif task == 'circle':
    ...
    robot_trajectory = path.to_robot_trajectory(num_way, controller_name!='workspace')
    return trajectory, robot_trajectory
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        ...
        # YOUR CODE HERE
        Kp = 0.2 * np.array([0.4, 2.0, 1.7, 1.5, 2.0, 2.0, 3.0])
        Kv = 0.1 * np.array([2.0, 1.0, 2.0, 1.5, 0.8, 0.8, 0.8])
...
def main():
    ...
    trajectory, robot_trajectory = get_trajectory(limb, kin, ik_solver, tag_pos, args)
    ...
    if args.controller_name == "moveit":
        ...
        #     log=args.log
        # )
        done  = controller.follow_ar_tag(tag=args.ar_marker[0], trajectory=trajectory, rate=args.rate, timeout=args.timeout, log=args.log)
        if not done:
