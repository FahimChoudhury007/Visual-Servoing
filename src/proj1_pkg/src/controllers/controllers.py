...
class Controller:
    ...
    def follow_ar_tag(self, trajectory, timeout=None, log=False):
        ...
        # For interpolation
        max_index = len(trajectory.joint_trajectory.points)-1
        current_index = 0
        ...
        while not rospy.is_shutdown():
            ...
            (
                ...
                target_acceleration, 
                current_index
            ) = self.interpolate_path(trajectory, t, current_index)
...
class WorkspaceVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
      ...
      inverse_kine  a quaternion, all written in spatial coordinates.
        ...
        """
        current_position = self._kin.forward_position_kinematics(get_joint_positions(self._limb))
        target_position = self._kin.forward_position_kinematics(target_position) 

        g_st = utils.hat(current_position)
        g_sd = utils.hat(target_position)
        g_td = np.dot(np.linalg.inv(g_sd), g_st)
    
        log_td = np.log(g_td)
        v_td = log_td[0:3][:3]
        w_td = np.array((log_td[2][1], log_td[0][2], log_td[1][0]))
        psi_td = None
        psi_std = None
        u_s = self.kp * psi_std + target_velocity
        control_input =  jacobian_pseudo_inverse(u_s)
...
class PDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        # control_input = target_velocity + np.dot(self.Kp, e) + np.dot(self.Kv, e_dot)
        control_input = target_velocity + np.dot(self.Kp, e)
        self._limb.set_joint_velocities(joint_array_to_dict(control_input, self._limb))
...
class PDJointTorqueController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        get_joint_positions and get_joint_velocities from the utils package to get the current joint 
        position and velocself.start_position = start_position
        ...
        self.desired_orientation = np.array([0, 1, 0, 0])ity and self._limb.set_joint_torques() to set the joint torques to something. 
        You may find joint_array_to_dict() in utils.py useful as well.
        ...
        positions_dict = joint_array_to_dict(current_position)
        velocity_dict = joint_array_to_dict(current_velocity)

        inertia = self._kin.inertia(positions_dict)
        coriolis = self._kin.coriolis(positions_dict, velocity_dict)
        gravity = self._kin.gravity(positions_dict)

        e =  target_position - current_position
        e_dot = target_velocity - current_velocity

        d =  target_acceleration - self.kv * e_dot - self.kp * e
    
        control_input = np.dot(inertia, d) + np.dot(coriolis, current_velocity) + gravity
